...
function freshSnapshotTimestamp(previousTimestamp){
  const now=new Date();
  const previousMs=Date.parse(previousTimestamp||'');
  if(Number.isFinite(previousMs)&&now.getTime()<=previousMs) now.setTime(previousMs+1);
...
function normalizeHeaderName(header){
  ...
  return ({ticker:'symbol',sym:'symbol',security:'symbol',last:'close',lastprice:'close',closeprice:'close',vol:'volume'})[normalized]||normalized;
}
function normalizePrice(value){
  if(typeof value==='number') return value;
  const cleaned=String(value??'').trim().replace(/,/g,'');
  return cleaned&&cleaned.toUpperCase()!=='N/D'?Number(cleaned):NaN;
}
function mapQuote(mapped, expectedTicker){
  const symbol=String(mapped.symbol||'').replace(/\.US$/i,'').toUpperCase();
  const open=normalizePrice(mapped.open);
  const close=normalizePrice(mapped.close);
  const volume=normalizePrice(mapped.volume);
  if(symbol!==expectedTicker) throw rejectionError('ticker_mismatch', `Ticker mismatch: expected ${expectedTicker}, got ${symbol||'empty'} (raw ${mapped.symbol||'empty'})`);
  if(!Number.isFinite(close)||close<=0) throw rejectionError('invalid_close_price', `Invalid close price: ${mapped.close??'empty'}`);
  const move=Number.isFinite(open)&&open>0?((close-open)/open*100):0;
  return {ticker:expectedTicker,name:quoteName(expectedTicker),sector:sectorByTicker[expectedTicker]||'Market scan',price:close,move:Number(move.toFixed(2)),volume:Number.isFinite(volume)?volume:0,source:'stooq',sourceSymbol:mapped.symbol,quoteDate:mapped.date,quoteTime:mapped.time,status:'ok',valid:true,downloaded:true};
}
async function quoteBatch(batch){
  if (MOCK_QUOTES) { refreshDiagnostics.sourceReached=true; return batch.map((ticker, i) => ({ticker,name:quoteName(ticker),sector:sectorByTicker[ticker]||'Market scan',price:Number((50+(i%25)*3+(ticker.charCodeAt(0)%17)).toFixed(2)),move:Number((((ticker.charCodeAt(0)+ticker.length)%9)-4).toFixed(2)),volume:1000000+i,source:'stooq-mock',sourceSymbol:`${ticker}.US`,quoteDate:'2026-05-31',quoteTime:'00:00:00',status:'ok',valid:true,downloaded:true})); }
  ...
  return batch.map(t=>out.get(t)||invalidQuote(t,'No CSV row returned', {status:'missing',validationRule:'missing_csv_row'}));
}
...
async function quoteBatchSafe(batch){
  ...
  if (lastError&&isSourceUnavailable(lastError)) {
    ...
    return batch.map(t=>invalidQuote(t,message, {validationRule:'source_unavailable'}));
  }
...
const quoteDownloadSucceeded=downloadedCount>0;
const lastSuccessfulMarketFetchAt=quoteDownloadSucceeded?generatedAt:(existing&&existing.lastSuccessfulMarketFetchAt)||'';
const lastPriceChangeAt=quoteDownloadSucceeded?generatedAt:((existing&&(existing.lastPriceChangeAt||existingUpdatedAt))||generatedAt);
const lastMarketQuoteTime=latestQuoteTimestamp(validQuotes)||(quoteDownloadSucceeded?generatedAt:(existing&&existing.lastMarketQuoteTime)||'');
...
refreshDiagnostics.timestampWillAdvance=timestampAdvanced&&(quoteDownloadSucceeded||!(existing&&Number(existing.validQuotesCount||existing.universeSize||0) >= MIN_VALID_QUOTES));
...
if (acceptedCount < MIN_VALID_QUOTES && !MOCK_QUOTES && downloadedCount === 0) {
