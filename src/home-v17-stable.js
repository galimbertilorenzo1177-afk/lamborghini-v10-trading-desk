(function(){
  ...
  function isLegacyCard(c){var txt=c.textContent||'';if(c.id==='home-v17-stable')return false;return txt.indexOf('Operating desk con freshness')>=0||txt.indexOf('Profilo capitale salvato localmente')>=0||txt.indexOf('NO TRADE: market.json oltre 60 minuti')>=0||(txt.indexOf('Regime breadth')>=0&&txt.indexOf('leverage forbidden')>=0)}
  function hideLegacy(){if(page()!=='home')return;var cards=document.querySelectorAll('.page > .card');for(var i=0;i<cards.length;i++){if(isLegacyCard(cards[i]))cards[i].style.display='none'}}
  ...
  function schedule(force){if(scheduled)return;scheduled=true;setTimeout(function(){scheduled=false;apply(!!force)},80)}
  ...
  function start(){watchPage();var i=0,t=setInterval(function(){i++;apply(i===1);if(i>=8)clearInterval(t)},250)}
  start();
  ...
  setInterval(function(){apply(false)},1500);
})();
