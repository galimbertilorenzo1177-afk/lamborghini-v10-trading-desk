...
function hideOldHomeCards(){var cards=document.querySelectorAll('.page > .card.hero');for(var i=0;i<cards.length;i++){var txt=cards[i].textContent||'';if(cards[i].id!=='v17-synth-agent'&&(txt.indexOf('Operating desk')>=0||txt.indexOf('Cosa fare oggi')>=0))cards[i].style.display='none'}}
...
function boot(){var tries=0;var timer=setInterval(function(){tries++;inject(tries===1);if(tries>=8||document.querySelector('#v17-synth-agent'))clearInterval(timer)},600)}
