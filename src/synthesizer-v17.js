...
function hideOldHomeCards(){var cards=document.querySelectorAll('.page > .card.hero,.page > .card.today');for(var i=0;i<cards.length;i++){var txt=cards[i].textContent||'';if(cards[i].id!=='v17-synth-agent'&&(txt.indexOf('Operating desk')>=0||txt.indexOf('Cosa fare oggi')>=0||txt.indexOf('selettivo')>=0))cards[i].style.display='none'}}
...
function inject(force){if(page()!=='home')return;var pg=document.querySelector('.page');if(!pg)return;var ma=macro(),rs=rows();if(!rs.length)return;var key=ma+'|'+rs.map(function(x){return x.t+':'+x.pl.toFixed(2)}).join('|');if(!force&&key===lastKey&&document.querySelector('#v17-synth-agent')){hideOldHomeCards();return}lastKey=key;var old=document.querySelector('#v17-synth-agent');if(old){old.outerHTML=build(rs,ma);hideOldHomeCards();return}var w=document.createElement('div');w.innerHTML=build(rs,ma);if(w.firstElementChild)pg.insertBefore(w.firstElementChild,pg.firstElementChild);hideOldHomeCards()}
function boot(){var tries=0;var timer=setInterval(function(){tries++;inject(tries===1);hideOldHomeCards();if(tries>=12)clearInterval(timer)},500)}
function afterRefresh(){lastKey='';setTimeout(boot,250);setTimeout(function(){inject(true)},1500);setTimeout(function(){inject(true)},3000);setTimeout(function(){inject(true)},5000)}
window.addEventListener('load',boot);
window.addEventListener('hashchange',function(){lastKey='';setTimeout(boot,250)});
window.addEventListener('storage',function(){lastKey='';setTimeout(function(){inject(true)},250)});
document.addEventListener('click',function(){afterRefresh()},true);
setInterval(function(){if(page()==='home'){inject(false);hideOldHomeCards()}},2000);
})();
