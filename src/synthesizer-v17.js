...
function hideOldHomeCards(){var cards=document.querySelectorAll('.page > .card.hero');for(var i=0;i<cards.length;i++){var txt=cards[i].textContent||'';if(cards[i].id!=='v17-synth-agent'&&(txt.indexOf('Operating desk')>=0||txt.indexOf('selettivo')>=0))cards[i].style.display='none'}}
...
function boot(){var tries=0;var timer=setInterval(function(){tries++;inject(tries===1);if(tries>=12)clearInterval(timer)},500)}
function afterRefresh(){lastKey='';setTimeout(boot,400);setTimeout(function(){inject(true)},1800);setTimeout(function(){inject(true)},3200)}
...
document.addEventListener('click',function(e){var x=e.target;if(!x)return;if(x.id==='refresh'||(x.closest&&x.closest('#refresh')))afterRefresh()},true);
})();
